    ...
    },
{
  ...
  description: "Terraform monorepo that eliminates always-on CI/CD servers. When a GitHub workflow job is queued, a webhook hits API Gateway → a Node.js Lambda validates the HMAC-SHA256 signature → SQS buffers the event → a runner manager Lambda authenticates via GitHub App, tries Spot pricing first (on-demand fallback), and launches an ephemeral EC2 instance that registers as a runner, completes the job, and self-terminates. Hash-based multi-AZ subnet selection spreads Spot capacity risk. DRY: one reusable GitHub Actions workflow drives all 5 Terraform modules via OIDC — no static AWS keys anywhere.\n\nThe AMI builder is a dedicated Terraform module that creates a pre-baked runner image so cold-start stays under 60 seconds. It spins up a temporary EC2 instance, delivers an install.sh script via user data, and uses SSM — no SSH, no open inbound ports — to run it. The script pre-installs everything a runner needs: Docker, Node.js 20 LTS, Terraform 1.10.5, tflint, Checkov, git, curl, jq, and Python 3. The module polls SSM until installation is confirmed (up to 60 retries), then snapshots the instance into an AMI, stores the AMI ID in SSM Parameter Store for the runner module to consume, and terminates the builder. Result: every ephemeral runner boots from a fully provisioned image with zero manual setup.",
